  ...
  return {
    list: (req, res) => {
      try {
        ...
        const { items, total, page, pageSize } = characterLibraryService.listLibraryItems(db, query, userScope);
    ...
    create: (req, res) => {
      try {
        const item = characterLibraryService.createLibraryItem(db, log, req.body || {}, req.user.id);
